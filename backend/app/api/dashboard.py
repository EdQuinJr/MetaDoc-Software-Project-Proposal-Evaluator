...
class DashboardService:
    ...
    def delete_deadline(self, deadline_id, user_id):
        """Delete a deadline and all associated submissions"""
        try:
            deadline = Deadline.query.filter_by(
                id=deadline_id,
                professor_id=user_id
            ...
            # Get all submissions linked to this deadline before deletion
            submissions = Submission.query.filter_by(deadline_id=deadline_id).all()
            submission_count = len(submissions)
            
            # Delete associated files and related data for each submission
            for submission in submissions:
                # Delete associated file
                if submission.file_path and os.path.exists(submission.file_path):
                    try:
                ...
                # Delete document snapshots
                DocumentSnapshot.query.filter_by(submission_id=submission.id).delete()
                
                # Delete audit logs
                AuditLog.query.filter_by(submission_id=submission.id).delete()
                
                # Explicitly delete the submission from database
                db.session.delete(submission)
            
            # Delete the deadline folder if it exists
            if deadline.title:
                import re
                folder_name = re.sub(r'[<>:"/\\|?*]', '_', deadline.title)
                folder_name = folder_name.strip()[:100]
                deadline_folder = os.path.join(current_app.config['UPLOAD_FOLDER'], folder_name)
                
                if os.path.exists(deadline_folder) and os.path.isdir(deadline_folder):
                    try:
                        import shutil
                        shutil.rmtree(deadline_folder)
                        current_app.logger.info(f"Deleted deadline folder: {deadline_folder}")
                    except Exception as e:
                        current_app.logger.warning(f"Could not delete folder {deadline_folder}: {e}")
            ...
            AuditService.log_event(
                ...
                description=f'Deadline and {submission_count} submission(s) deleted: {deadline.title}',
                user_id=user_id,
                metadata={
